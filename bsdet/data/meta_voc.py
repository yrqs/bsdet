...
def load_filtered_voc_instances(
    name: str, dirname: str, split: str, classnames: str, sid: int
):
    """
    ...
    if is_shots:
        ...
        for cls in classnames:
            if sid == 9 and (not playback):
                if cls in classnames[:-5]:
                    continue
            elif sid >= 4 and (not playback):
                if cls in classnames[:-1]:
                    continue
...
def register_meta_voc(
    ...
    DatasetCatalog.register(
        ...
        lambda: load_filtered_voc_instances(
            name, dirname, split, thing_classes, sid
        ),
    )
