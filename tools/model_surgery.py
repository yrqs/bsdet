...
cls_score_name = ['cls_score', 'reps'][1]
...
def main(args):
    ...
    def surgery(param_name, is_weight, tar_size, ckpt):
        ...
        if weight_name not in ckpt['model'].keys():
            return
        pretrained_weight = ckpt['model'][weight_name]
        prev_cls = pretrained_weight.size(0)
        if '.' + cls_score_name in param_name:
            prev_cls -= 1
        if is_weight:
        ...
        if args.dataset == 'coco':
            for idx, c in enumerate(BASE_CLASSES):
                ...
                elif 'neg_reps' in param_name:
                    new_weight[IDMAP[c]*NEG_MODE:(IDMAP[c]+1)*NEG_MODE] = \
                        pretrained_weight[idx*NEG_MODE:(idx+1)*NEG_MODE]
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--param-name', type=str, nargs='+', help='Target parameter names',
                        default=['roi_heads.box_predictor.' + cls_score_name, 'roi_heads.box_predictor.neg_reps', 'roi_heads.box_predictor.bbox_pred'])
    # parser.add_argument('--param-name', type=str, nargs='+', help='Target parameter names',
    #                     default=['roi_heads.box_predictor.' + cls_score_name, 'roi_heads.box_predictor.neg_reps'])
    # parser.add_argument('--param-name', type=str, nargs='+', help='Target parameter names',
    #                     default=['roi_heads.box_predictor.' + cls_score_name, ])
